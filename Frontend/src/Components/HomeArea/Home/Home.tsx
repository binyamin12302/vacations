...
function Home(): JSX.Element {
    ...
    async function handleFollowVacation(follow: FollowModel, isFollowing: string) {
        try {
        ...
        <Container className="Home  p-2">
            ...
            {authService.isUserAdmin() &&
                <div className="col-md-12  adm text-center">
                    ...
                    <Button variant="light" className="border btn-vac border-secondary shadow-none"
                        onClick={handleShowModalAddVacation}
                    >Add Vacation</Button>

                    <NavLink className="btn btn-secondary text-white shadow-none" to="/chart">
                        Go to the Chart
                    </NavLink>


                    <Modal show={showModalAddVacation} onHide={handleCloseModalAddVacation}>
            ...
            <Row xs={1} sm={1} md={2} lg={3} xl={5} className="g-4 m-auto" >
                {
                    currentVacations.map(v =>
                        <VacationCard
                            handleFollow={handleFollowVacation} key={v.id} vacation={v} />
                    )
                }
            </Row>
